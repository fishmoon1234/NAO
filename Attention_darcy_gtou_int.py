...
def main():
    ...
    X_TRAIN_PATH = './darcy_data/DarcyStatic_A100F100_10000x21x21_chi_sol_source.mat'
    ...
    f_train = reader.read_field('source')[train_list, :].view(ntrain*sample_per_task, gridN, gridN)

    sol_test = reader.read_field('sol')[test_list, :].view(ntest*sample_per_task, gridN, gridN)
    f_test = reader.read_field('source')[test_list, :].view(ntest*sample_per_task, gridN, gridN)
    ...
    train_loader = torch.utils.data.DataLoader(torch.utils.data.TensorDataset(f_train, x_train, xy_train),batch_size=batch_size, shuffle=True)
    test_loader = torch.utils.data.DataLoader(torch.utils.data.TensorDataset(f_test, x_test, xy_test),batch_size=batch_size, shuffle=False)
    ...
    model_u.load_state_dict(torch.load('./best_model_dk40_darcy_gtou.ckpt', map_location="cpu"))
